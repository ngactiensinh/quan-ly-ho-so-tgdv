...
# ==========================================
# CSS PHONG CÁCH CHÍNH TRỊ - ĐỎ & VÀNG
# ==========================================
st.markdown("""
<style>
    ...
    /* ---- BACKGROUND ---- */
    .stApp {
        background: linear-gradient(160deg, #f8f4ef 0%, #ede8e0 50%, #f5f0e8 100%);
    }

    /* ---- SIDEBAR ---- */
    [data-testid="stSidebar"] {
        background: linear-gradient(180deg, var(--navy) 0%, var(--navy-mid) 60%, #0D1E35 100%) !important;
    ...
    /* ---- HEADER ---- */
    .gov-header {
        background: linear-gradient(135deg, var(--navy) 0%, var(--navy-mid) 40%, var(--red-dark) 100%);
    ...
    .profile-card::before {
        ...
        background: linear-gradient(135deg, rgba(200,16,46,0.04), rgba(212,175,55,0.06));
    ...
    .profile-name { font-family: 'Merriweather', serif; font-size: 26px; font-weight: 900; color: var(--navy); letter-spacing: 0.5px; margin-bottom: 4px; text-transform: uppercase; }
    ...
    div[data-testid="stDownloadButton"] > button:hover {
        ...
        box-shadow: 0 5px 18px rgba(200,16,46,0.4) !important;
    ...
    .stTabs [data-baseweb="tab-list"] {
        ...
        border-bottom: 2px solid rgba(200,16,46,0.15) !important;
        gap: 4px !important;
    }
    ...
    /* ---- ALERTS ---- */
    [data-testid="stSuccess"] { background: rgba(46,125,50,0.08) !important; border-left: 4px solid #2E7D32 !important; border-radius: 6px !important; }
    [data-testid="stError"] { background: rgba(200,16,46,0.08) !important; border-left: 4px solid var(--red-primary) !important; border-radius: 6px !important; }
    [data-testid="stWarning"] { background: rgba(212,175,55,0.1) !important; border-left: 4px solid var(--gold) !important; border-radius: 6px !important; }
    [data-testid="stInfo"] { background: rgba(10,22,40,0.06) !important; border-left: 4px solid var(--navy-mid) !important; border-radius: 6px !important; }
    ...
    .user-chip .name { font-weight: 700; font-size: 14px; color: var(--gold-light); }
...
if not st.session_state["logged_in"]:
    ...
    <div style="max-width:900px; margin: 0 auto 32px auto;">
        <div style="background: linear-gradient(135deg, {chr(10)}var(--navy, #0A1628) 0%, #1A2E4A 50%, #C8102E 100%);
...
<style>
  ...
  body {{ font-family: 'Source Sans 3', 'Times New Roman', serif; line-height: 1.7; padding: 48px 56px; max-width: 860px; margin: auto; color: #1a1a2e; font-size: 15px; }}
  .header {{ text-align: center; border-bottom: 3px double #C8102E; padding-bottom: 20px; margin-bottom: 30px; }}
  ...
  .header .org {{ color: #C8102E; font-size: 14px; font-weight: 600; }}
  .info-grid {{ display: grid; grid-template-columns: 1fr 1fr; gap: 10px 30px; margin: 16px 0; }}
  .info-grid p {{ margin: 0; padding: 6px 0; border-bottom: 1px dotted #ddd; }}
  h3 {{ font-family: 'Merriweather', serif; color: #0A1628; background: linear-gradient(90deg, #f5e8e8, #fff); padding: 8px 14px; border-left: 5px solid #C8102E; margin: 30px 0 14px; font-size: 14px; text-transform: uppercase; letter-spacing: 1px; }}
  table {{ width: 100%; border-collapse: collapse; font-size: 13px; }}
  th {{ background: #0A1628; color: #fff; padding: 9px 12px; text-align: left; font-size: 12px; letter-spacing: 0.5px; }}
  td {{ padding: 8px 12px; border-bottom: 1px solid #e8e0d4; vertical-align: top; }}
  tr:nth-child(even) td {{ background: #fdf8f0; }}
  .footer {{ margin-top: 50px; text-align: right; font-size: 13px; color: #888; border-top: 1px solid #ddd; padding-top: 16px; }}
</style></head><body>
<div class="header">
...
<div class="info-grid">
  <p><strong>Họ và tên:</strong> {str(info.get('ho_ten','')).upper()}</p>
  <p><strong>Mã CBCC:</strong> {info.get('id','')}</p>
  <p><strong>Ngày sinh:</strong> {info.get('ngay_sinh','')}</p>
  <p><strong>Giới tính:</strong> {info.get('gioi_tinh','')}</p>
  <p><strong>Quê quán:</strong> {info.get('que_quan','')}</p>
  <p><strong>Đơn vị:</strong> {info.get('don_vi','')}</p>
  <p><strong>Chức vụ:</strong> {info.get('chuc_vu','')}</p>
  <p><strong>Ngạch công chức:</strong> {info.get('ngach_cong_chuc','')}</p>
  <p><strong>Trình độ chuyên môn:</strong> {info.get('trinh_do_chuyen_mon','')}</p>
  <p><strong>Lý luận chính trị:</strong> {info.get('ly_luan_chinh_tri','')}</p>
  <p><strong>Ngày kết nạp Đảng:</strong> {info.get('ngay_vao_dang','')}</p>
  <p><strong>Ngày chính thức:</strong> {info.get('ngay_chinh_thuc','')}</p>
</div>
...
if menu == "📊 Dashboard":
    ...
    if df_hoso.empty:
        ...
        dang_vien = len(df[df["ngay_vao_dang"].astype(str).str.len() > 3])
        ...
        colors_red_gold = ['#C8102E', '#D4AF37', '#1A2E4A', '#E8203F', '#F0D060', '#2A4A70']
        ...
        with col1:
            ...
            fig_gt.update_traces(textposition='outside', textinfo='percent+label',
                                 marker=dict(line=dict(color='#fff', width=3)))
            fig_gt.update_layout(font_family="Source Sans 3", title_font_size=15,
                                 plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)')
            st.plotly_chart(fig_gt, use_container_width=True)

            df_ng = df['ngach_cong_chuc'].value_counts().reset_index()
            df_ng.columns = ['Ngạch', 'Số lượng']
            fig_ng = px.bar(df_ng, x='Ngạch', y='Số lượng', title='<b>Ngạch Công chức hiện hưởng</b>',
        ...
        with col2:
            ...
            fig_ll = px.bar(df_ll, x='Lý luận CT', y='Số lượng',
        ...
        df_show = df[[c for c in cols_show if c in df.columns]].rename(columns={
            ...
            'trinh_do_chuyen_mon': 'Chuyên môn', 'ly_luan_chinh_tri': 'Lý luận CT'
        })
                ...
                for idx, row in df_choduyet.iterrows():
                    with st.expander(f"👤  {row['ho_ten']}  ({row['ma_cbcc']})  —  {row['chuc_vu']}"):
                        ...
                        if c_duyet.button("✅  PHÊ DUYỆT TÀI KHOẢN", key=f"duyet_{row['ma_cbcc']}", use_container_width=True):
                            ...
                            st.success("✅ Đã phê duyệt tài khoản.")
                            st.rerun()
                ...
                if st.button("🗑️  XÁC NHẬN XÓA VĨNH VIỄN", use_container_width=True):
                    if del_ma != "— Chọn tài khoản —":
                        ...
                        if ma_xoa.upper() == "ADMIN":
                            ...
                            st.success(f"✅ Đã xóa tài khoản {ma_xoa}.")
                            st.rerun()
    ...
    if df_hoso.empty:
        ...
        if ma_chon:
            ...
            col_dl.download_button(
                label="📥  TẢI SƠ YẾU LÝ LỊCH (BẢN IN)",
                data=html_data,
                file_name=f"SYLT_{info['ho_ten'].replace(' ', '_')}_{info['id']}.html",
                mime="text/html",
            ...
            <div class="profile-card">
                <div class="profile-badge">CÁN BỘ CÔNG CHỨC</div>
                <div class="profile-name">{info['ho_ten']}</div>
                <div class="profile-title">🏛️ {info.get('chuc_vu','—')} &nbsp;|&nbsp; {info.get('don_vi','—')}</div>
                <hr class="profile-divider">
                <div class="profile-info-grid">
                    <div class="info-item"><span class="lbl">Mã CBCC</span><span class="val">{info.get('id','—')}</span></div>
                    ...
                    <div class="info-item"><span class="lbl">Ngày vào Đảng</span><span class="val">KN: {info.get('ngay_vao_dang','—')} &nbsp; CT: {info.get('ngay_chinh_thuc','—')}</span></div>
                </div>
            </div>
            """, unsafe_allow_html=True)
            ...
            with t_ct:
                if not df_ct.empty:
                    st.dataframe(df_ct.rename(columns={'tu_ngay': 'Từ ngày', 'den_ngay': 'Đến ngày', 'vi_tri': 'Vị trí', 'don_vi': 'Đơn vị', 'quyet_dinh_so': 'Quyết định số'}), hide_index=True, use_container_width=True)
                else:
                    st.info("Chưa có dữ liệu lịch sử công tác.")
            with t_l:
                if not df_l.empty:
                    st.dataframe(df_l.rename(columns={'ngay_quyet_dinh': 'Ngày QĐ', 'bac_luong': 'Bậc lương', 'he_so': 'Hệ số', 'quyet_dinh_so': 'Quyết định số'}), hide_index=True, use_container_width=True)
                else:
                    st.info("Chưa có dữ liệu diễn biến lương.")
            with t_kt:
                if not df_kt.empty:
                    st.dataframe(df_kt.rename(columns={'ngay_quyet_dinh': 'Ngày QĐ', 'loai': 'Loại', 'noi_dung': 'Nội dung', 'quyet_dinh_so': 'Quyết định số'}), hide_index=True, use_container_width=True)
                else:
                    st.info("Chưa có dữ liệu khen thưởng / kỷ luật.")
            with t_gd:
                if not df_gd.empty:
                    df_gd_show = df_gd.drop(columns=['id', 'ma_cbcc', 'created_at', 'thong_tin_khac'], errors='ignore').rename(columns={
                        ...
                        'nam_sinh': 'Năm sinh', 'que_quan_gd': 'Quê quán',
                        'nghe_nghiep_gd': 'Nghề nghiệp / Công tác', 'noi_o_gd': 'Nơi ở hiện nay'
                    })
...
# ==========================================
    ...
    tab_chinh, tab_ct, tab_luong, tab_kt, tab_gd = st.tabs([
        "👤  Hồ sơ chính", "🏢  Lịch sử công tác",
        "💰  Diễn biến lương", "🏆  Khen thưởng / Kỷ luật", "👨‍👩‍👧‍👦  Quan hệ gia đình"
    ])
    ...
    with tab_chinh:
        with st.form("form_ho_so", clear_on_submit=False):
            ...
            with c3:
                ...
                cp1, cp2 = st.columns(2)
                ngay_ket_nap = cp1.text_input("Ngày kết nạp", value=ex_data.get("ngay_vao_dang", ""))
                ngay_chinh_thuc = cp2.text_input("Ngày chính thức", value=ex_data.get("ngay_chinh_thuc", ""))
            ...
            if st.form_submit_button("💾  LƯU HỒ SƠ CHÍNH", use_container_width=True):
                if not target_id or not ho_ten:
                    ...
                    data = {
                        ...
                        "chuc_vu": chuc_vu, "ngach_cong_chuc": ngach,
                        "trinh_do_chuyen_mon": chuyen_mon, "ly_luan_chinh_tri": ly_luan,
                        "ngay_vao_dang": ngay_ket_nap, "ngay_chinh_thuc": ngay_chinh_thuc
                    }
                    supabase.table("ho_so_cbcc").upsert(data).execute()
                    st.session_state["edit_target_id"] = ""
                    st.success("✅ Đã lưu hồ sơ chính thành công.")
                    st.cache_data.clear()
                    st.rerun()

    def render_sub_tab(table_name, form_key, title_btn, fields_form, display_cols, rename_map):
        with st.form(f"form_{form_key}"):
            ...
            for i, (fname, flabel) in enumerate(fields_form):
                with cols_form[i % 2]:
                    vals[fname] = st.text_input(flabel)
        ...
        try:
            df_sub = pd.DataFrame(supabase.table(table_name).select("*").eq("ma_cbcc", target_id).order("id").execute().data)
        except Exception:
            df_sub = pd.DataFrame()

        if not df_sub.empty:
            edited = st.data_editor(df_sub.drop(columns=['ma_cbcc', 'created_at'], errors='ignore'),
            ...
            if cs.button(f"💾  LƯU THAY ĐỔI — {title_btn.upper()}", use_container_width=True, key=f"save_{form_key}"):
                upd = edited.copy()
                upd['ma_cbcc'] = target_id
                supabase.table(table_name).upsert(upd.fillna("").to_dict(orient="records")).execute()
                st.success("✅ Đã lưu.")
                st.rerun()
            del_id = cd.selectbox("ID cần xóa:", ["—"] + df_sub['id'].astype(str).tolist(),
                                  label_visibility="collapsed", key=f"del_sel_{form_key}")
            ...
            st.info("Chưa có dữ liệu. Hãy thêm mới ở phía trên.")

    with tab_ct:
        render_sub_tab(
            "lich_su_cong_tac", "cong_tac", "Lịch sử công tác",
            [("tu_ngay","Từ ngày"), ("den_ngay","Đến ngày"), ("vi_tri","Vị trí / Chức danh"), ("don_vi","Đơn vị công tác"), ("quyet_dinh_so","Quyết định số")],
            [], {}
        )

    with tab_luong:
        render_sub_tab(
            "dien_bien_luong", "luong", "Diễn biến lương",
            [("ngay_quyet_dinh","Ngày quyết định"), ("bac_luong","Bậc lương"), ("he_so","Hệ số"), ("quyet_dinh_so","Quyết định số")],
            [], {}
        )
    ...
    with tab_kt:
        with st.form("form_kt"):
            ...
            if st.form_submit_button("➕  THÊM MỚI KHEN THƯỞNG / KỶ LUẬT", use_container_width=True):
                supabase.table("khen_thuong_ky_luat").insert({
                    "ma_cbcc": target_id, "ngay_quyet_dinh": ngay_qd, "loai": loai,
                    "noi_dung": noi_dung, "quyet_dinh_so": qd_so
                }).execute()
                st.success("✅ Đã thêm.")
                st.rerun()
        ...
        if not df_kt2.empty:
            edited_kt = st.data_editor(df_kt2.drop(columns=['ma_cbcc', 'created_at'], errors='ignore'),
                                       hide_index=True, use_container_width=True, disabled=["id"])
            ...
            if cs3.button("💾  LƯU THAY ĐỔI — KHEN THƯỞNG / KỶ LUẬT", use_container_width=True, key="save_kt"):
                ...
                st.success("✅ Đã lưu.")
                st.rerun()
            del_kt = cd3.selectbox("ID cần xóa:", ["—"] + df_kt2['id'].astype(str).tolist(),
                                   label_visibility="collapsed", key="del_kt")
            if cd3.button("🗑️  XÓA", key="del_btn_kt", use_container_width=True) and del_kt != "—":
                supabase.table("khen_thuong_ky_luat").delete().eq("id", del_kt).execute()
                st.rerun()
    ...
    with tab_gd:
        ...
        try:
            ...
            if not df_gd2.empty:
                edited_gd = st.data_editor(df_gd2.drop(columns=['ma_cbcc', 'created_at', 'thong_tin_khac'], errors='ignore'),
                                           hide_index=True, use_container_width=True, disabled=["id"])
                ...
                if cs4.button("💾  LƯU THAY ĐỔI — QUAN HỆ GIA ĐÌNH", use_container_width=True, key="save_gd"):
                    ...
                    st.success("✅ Đã lưu.")
                    st.rerun()
                del_gd = cd4.selectbox("ID cần xóa:", ["—"] + df_gd2['id'].astype(str).tolist(),
                                       label_visibility="collapsed", key="del_gd")
                if cd4.button("🗑️  XÓA", key="del_btn_gd", use_container_width=True) and del_gd != "—":
                    supabase.table("quan_he_gia_dinh").delete().eq("id", del_gd).execute()
                    st.rerun()
            else:
                st.info("Chưa có dữ liệu. Thêm người thân ở phía trên.")
        except Exception as e:
            st.warning(f"⚠️ Không tải được dữ liệu gia đình. Kiểm tra cấu trúc bảng Supabase. Lỗi: {e}")
