                ...
                for idx, row in df_choduyet.iterrows():
                    with st.expander(f"👤  {row['ho_ten']}  ({row['ma_cbcc']})  —  {row['chuc_vu']}"):
                        ...
                        if c_duyet.button("✅  PHÊ DUYỆT TÀI KHOẢN", key=f"duyet_{row['ma_cbcc']}", use_container_width=True):
                            ...
                            # Tự động tạo vỏ hồ sơ đi kèm 11 trường mới
                            try:
...
# ==========================================
    ...
    if df_hoso.empty:
        ...
        if ma_chon:
            ...
            # Profile card mở rộng hiển thị đầy đủ các trường mới bổ sung
            st.markdown(f"""
...
# ==========================================
# MODULE 4: NHẬP LIỆU & CHỈNH SỬA (BỔ SUNG 11 TRƯỜNG MỚI)
# ==========================================
